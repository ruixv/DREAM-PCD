...
def cal_loss(pred, gold, smoothing=False, use_focal_loss=False, 
             gamma=2.0, alpha=0.25, ignore_index=255):
    ''' 
    Calculate loss. Can be Cross-Entropy, Label Smoothing, or Focal Loss.
    '''
    
    gold = gold.contiguous().view(-1)

    if use_focal_loss:
        not_ignored = gold != ignore_index
        pred = pred[not_ignored]
        gold = gold[not_ignored]
        
        if pred.shape[0] == 0:
            return torch.tensor(0., device=pred.device, requires_grad=True)

        ce_loss = F.cross_entropy(pred, gold, reduction='none', ignore_index=ignore_index)
        
        log_softmax = F.log_softmax(pred, dim=1)
        pt = torch.exp(log_softmax.gather(1, gold.unsqueeze(1))).squeeze(1)
        
        modulating_factor = (1 - pt) ** gamma
        
        if alpha is not None:
            alpha_t = torch.full_like(gold, 1 - alpha, device=pred.device, dtype=torch.float32)
            # 获取目标类别的索引
            target_indices = (gold == 1).nonzero(as_tuple=True)[0]
            alpha_t[target_indices] = alpha
        ...
        not_ignored_mask = (gold != ignore_index).view(-1, 1)
        
        one_hot = one_hot * (1 - eps) + (1 - one_hot) * eps / (n_class - 1)
        log_prb = F.log_softmax(pred, dim=1)
        
        loss = -(one_hot * log_prb).sum(dim=1)
        loss = loss.masked_select(not_ignored_mask.squeeze()).mean()
        return loss
        
    else:
        return F.cross_entropy(pred, gold, reduction='mean', ignore_index=ignore_index)
